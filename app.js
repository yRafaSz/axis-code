...
async function requestCompleteAxisAnswer(baseMessages, progress) {
  ...
  const maxAutomaticContinuations = 16;
  for (let part = 0; part <= maxAutomaticContinuations; part++) {
    ...
    requestMessages = [
      ...
      {
        role: 'user',
        content: 'CONTINUAÇÃO_INTERNA_DE_PROJETO_GRANDE: continue exatamente do ponto onde parou, sem repetir nenhum texto ou arquivo já entregue. Termine todos os arquivos, códigos, configurações, instruções e o título interno que ainda faltarem. Não explique a continuação e não resuma: entregue diretamente o conteúdo restante.'
      }
    ];
  }
  throw new Error('O projeto excedeu o limite máximo de continuações automáticas. Divida-o em módulos menores.');
}
